...
- **Execute it.** On a failure: reproduce, **shrink** to the smallest input, commit it as a named regression test, and emit a findings entry marked `verified.verdict: upheld`. With no test infra, generate + present the harness clearly marked **not executed**.
