...
4. **Merge & triage** — pipe findings through `bughunt.py merge` (fingerprint, dedupe, cross-validate, suppress, baseline-diff) and score with the [triage](${CLAUDE_PLUGIN_ROOT}/skills/triage/SKILL.md) rubrics. Under `ci`, include `--require-verified --require-coverage --strict --fail-on critical-high`.
